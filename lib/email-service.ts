...
class EmailService {
    ...
    async sendVerificationEmail(email: string, code: string, name: string): Promise<boolean> {
        ...
        if (!transporter) {
            console.error('❌ SMTP non configuré (ni DB, ni ENV). Impossible d\'envoyer l\'email.');
            console.log(`📧 Code de vérification pour ${email}: ${code}`);
            return false;
        }

        const siteName = settings?.siteName || 'VideoAnnonces-CI';
        const siteSlogan = settings?.siteSlogan || 'La première plateforme de petites annonces vidéo en Côte d\'Ivoire';
